...
Goal: Build the "Truth Engine" using Anchor.
    ...
    [ ] State Definition: Create a FarmAccount PDA (Program Derived Address) that stores:
    ...
    [ ] The Mint Logic: Implement a mint_carbon_credits instruction.
    ...
    [ ] The Burn Logic: Implement a retire_credits instruction.
...
Goal: Python-based biomass calculation and cryptographic signing.
    ...
    [ ] FastAPI Wrapper: Ensure CORS is enabled so the sol-dapp-web can call it directly.

3. Phase 3: The Data Sync Layer (sb-server)

Goal: Fast data retrieval and WebSocket updates.

    [ ] Helius Webhook Listener: - Create a POST route /webhook to receive real-time updates from the Solana cluster.

        Filter for Burn events or Mint events related to the TerraNode Program ID.

    [ ] Caching Layer: - Use an in-memory Map or simple SQLite to store the "Global Carbon Locked" total so the frontend doesn't have to scan the whole chain.

    [ ] WebSockets: Broadcast "New Carbon Retired!" messages to connected frontend clients for the "Live Feed" UI component.

4. Phase 4: The Web Command Center (sol-dapp-web)

Goal: Corporate dashboard and Public "Blink" interface.

    [ ] Wallet Connection: Integrate @solana/wallet-adapter-react.
...
Goal: On-the-ground registration for farmers.
    ...
    [ ] Mobile Wallet Integration: Use transact from the Solana Mobile Stack (SMS) to connect to Phantom/Solflare.
