...
type CarbonEventType = "MINT" | "BURN";

interface CarbonLedgerEvent {
    signature: string;
    owner: string;
    amount: number;
    timestamp: number;
    eventType: CarbonEventType;
}

interface ParsedHeliusEvent {
    signature?: string;
    events?: {
        anchor?: {
            eventName?: string;
...
app.get("/server/health", function (_req: Request, res: Response) {
    res.status(200).json({
...
app.get("/metrics/global-offset", function (_req: Request, res: Response) {
    res.status(200).json(globalCache);
});

app.post("/webhooks/solana", async function (req: Request, res: Response) {
    const payload = req.body as ParsedHeliusEvent[] | ParsedHeliusEvent;
    const events = Array.isArray(payload) ? payload : [payload];

    const ledgerUpdates: CarbonLedgerEvent[] = [];
    for (const tx of events) {
        ...
        for (const anchorEvent of anchorEvents) {
            ...
            if (eventName === "CarbonRetired" && amount > 0) {
                const burnEvent: CarbonLedgerEvent = {
                    signature,
                    owner,
                    amount,
                    timestamp,
            ...
            if (eventName === "CarbonMinted" && amount > 0) {
                const mintEvent: CarbonLedgerEvent = {
                    signature,
                    owner,
                    amount,
                    timestamp,
...
async function appendEvents(newEvents: CarbonLedgerEvent[]): Promise<void> {
...
async function readExistingEvents(): Promise<CarbonLedgerEvent[]> {
    try {
        const data = await fs.readFile(ledgerFile, "utf8");
        const parsed = JSON.parse(data) as CarbonLedgerEvent[];
